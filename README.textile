...
A JavaScript Prototype Class that takes a sequence of images or HTML elements and turns them into a Slide Show. "View the Prototype SlideShow demo":http://homepage.mac.com/mattgarrett/prototype-slideshow/
...
|id + ':transitioned'|Fired when a slide has finished appearing from a transition.|Contains a reference to the SlideShow instance, the slide that faded, the slide that appeared, and the current loopCount.|
